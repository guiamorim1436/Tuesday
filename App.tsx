...
import { LayoutDashboard, Users, Settings, PieChart, Layers, Menu, X, ChevronLeft, ChevronRight, UserCog, LogOut, Package, Hexagon, BookOpen, MessageSquare } from 'lucide-react';
...
type View = 'dashboard' | 'conversas' | 'tasks' | 'clients' | 'finance' | 'settings' | 'users' | 'plans' | 'workflow' | 'playbooks';
...
const AppContent: React.FC<{ user: User, onLogout: () => void }> = ({ user, onLogout }) => {
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'conversas': return <ChatModule />;
    ...
    <div className="flex h-screen font-sans text-slate-900 overflow-hidden bg-[#F3F4F6]">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-xl border-b border-white/20 text-slate-900 flex items-center justify-between px-4 z-50 shadow-sm">
          <div className="flex items-center space-x-3">
              <img src="/logo.png" alt="Logo" className="h-10 w-auto rounded-lg shadow-sm" />
      ...
      <aside className={`fixed md:relative z-40 flex flex-col flex-shrink-0 transition-all duration-300 h-full border-r border-white/10 shadow-2xl ${isSidebarOpen ? 'translate-x-0 w-72' : '-translate-x-full md:translate-x-0'} ${isCollapsed ? 'md:w-24' : 'md:w-72'} pt-16 md:pt-0 bg-slate-900/95 backdrop-blur-xl text-white`}>
        <div className={`p-8 flex items-center ${isCollapsed ? 'justify-center' : 'space-x-5'} mb-4`}>
          <div className="flex-shrink-0 relative group">
             <div className="absolute -inset-2 bg-white/10 rounded-2xl blur opacity-25"></div>
             <div className="relative p-2 bg-white rounded-2xl shadow-xl">
                <img src="/logo.png" alt="Logo" className={`${isCollapsed ? 'h-8' : 'h-12'} w-auto object-contain transition-all`} />
             </div>
          </div>
          {!isCollapsed && (
            <div className="overflow-hidden whitespace-nowrap animate-in fade-in slide-in-from-left-4">
                <h1 className="text-xl font-bold tracking-tight text-white">Tuesday</h1>
                <p className="text-[10px] text-slate-400 tracking-[0.2em] font-black uppercase">Operation Hub</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          <NavItem view="conversas" icon={MessageSquare} label="Conversas" />
          ...
          <NavItem view="playbooks" icon={BookOpen} label="Wiki Processos" />
          
          {user.role === 'admin' && (
            <>
              <div className="my-4 border-t border-white/10 mx-2"></div>
              <NavItem view="plans" icon={Package} label="Planos & Pricing" />
        ...
        <div className="p-6 border-t border-white/5 bg-black/10">
          ...
          <button onClick={onLogout} className={`w-full flex items-center px-4 py-3 text-sm font-black rounded-xl transition-all duration-200 text-rose-400 hover:bg-rose-950/30 hover:text-rose-300 mt-2 ${isCollapsed ? 'justify-center px-2' : ''}`}>
             <LogOut size={20} className={isCollapsed ? '' : 'mr-3'}/>
             {!isCollapsed && <span>Sair</span>}
          </button>
          <button onClick={() => setIsCollapsed(!isCollapsed)} className="hidden md:flex items-center justify-center w-full mt-6 p-2 text-slate-500 hover:text-white transition-colors">
