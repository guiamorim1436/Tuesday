...
const AppContent: React.FC<{ user: User, onLogout: () => void }> = ({ user, onLogout }) => {
  ...
  return (
    <div className="flex h-screen font-sans text-slate-900 overflow-hidden bg-[#F3F4F6]">
      ...
      {/* Global Rich Gradient Background */}
      ...
      {/* Glass Sidebar */}
      <aside 
        className={`fixed md:relative z-40 flex flex-col flex-shrink-0 transition-all duration-300 h-full border-r border-white/10 shadow-2xl
        ${isSidebarOpen ? 'translate-x-0 w-72' : '-translate-x-full md:translate-x-0'} 
        ${isCollapsed ? 'md:w-24' : 'md:w-72'}
        pt-16 md:pt-0 bg-slate-900/95 backdrop-blur-xl text-white
        `}
      >
        <div className={`p-8 flex items-center ${isCollapsed ? 'justify-center' : 'space-x-4'} mb-2`}>
          ...
          {!isCollapsed && (
            <div className="overflow-hidden whitespace-nowrap">
                ...
                <p className="text-[10px] text-slate-400 tracking-[0.2em] font-medium uppercase">Tenno HUB</p>
            </div>
          )}
        </div>
        ...
        <div className="p-6 border-t border-white/5 bg-black/10">
          ...
          <button 
            onClick={onLogout}
            className={`w-full flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 text-rose-400 hover:bg-rose-950/30 hover:text-rose-300 mt-2 ${isCollapsed ? 'justify-center px-2' : ''}`}
          >
