...
export const api = {
            ...
            try {
                ...
                if (!error && data && data.length > 0) {
                    return data.map(t => ({ 
                        ...
                        clientId: t.client_id, status: t.status as TaskStatus, 
                        priority: t.priority as TaskPriority, category: t.category, 
                        startDate: t.start_date, dueDate: t.due_date, createdAt: t.created_at, 
                        estimated_hours: t.estimated_hours, actualHours: t.actual_hours, 
                        autoSla: t.auto_sla ?? true, isTrackingTime: t.is_tracking_time, 
                        lastTimeLogStart: t.last_time_log_start ? Number(t.last_time_log_start) : undefined, 
                        assignees: t.assignees || [], subscribers: t.subscribers || [],
                        participants: t.participants || [], watchers: t.watchers || [], 
                        customFields: t.custom_fields || {}, attachments: t.attachments || [], 
                        subtasks: [], comments: [], externalId: t.external_id, meetLink: t.meet_link
                    }));
                }
            } catch (e) { console.warn("Supabase tasks connection error."); }
        }
    ...
    getClients: async (): Promise<Client[]> => {
        if (isConfigured) {
            try {
                ...
                if (!error && data && data.length > 0) {
                    return data.map(c => ({ 
                        ...c, slaTierId: c.sla_tier_id, partnerId: c.partner_id, 
                        onboardingDate: c.onboarding_date, healthScore: c.health_score, 
                        hoursUsedMonth: c.hours_used_month, hasImplementation: c.has_implementation, 
                        billingDay: c.billing_day, customFields: c.custom_fields || {} 
                    }));
                }
            } catch (e) { console.warn("Supabase clients connection error."); }
        }
    ...
    getPartners: async (): Promise<Partner[]> => {
        if (isConfigured) {
            try {
                ...
                if (!error && data && data.length > 0) {
                    return data.map(p => ({ 
                        ...p, totalReferrals: p.total_referrals, totalCommissionPaid: p.total_commission_paid, 
                        implementationFee: p.implementation_fee, implementationDays: p.implementation_days, 
                        costPerSeat: p.cost_per_seat, customFields: p.custom_fields || {} 
                    }));
                }
            } catch (e) { console.warn("Supabase partners connection error."); }
        }
    ...
    getTransactions: async (): Promise<Transaction[]> => {
        if (isConfigured) {
            try {
                ...
                if (!error && data && data.length > 0) {
                    return data.map(tr => ({ 
                        ...tr, clientId: tr.client_id, partnerId: tr.partner_id, customFields: tr.custom_fields || {} 
                    }));
                }
            } catch (e) { console.warn("Supabase transactions connection error."); }
        }
    ...
    createTask: async (task: Partial<Task>): Promise<Task> => {
        if (isConfigured) {
             const payload = { 
                 title: task.title, description: task.description, client_id: toUUID(task.clientId), 
                 status: task.status, priority: task.priority, category: task.category, 
                 start_date: toDate(task.startDate), due_date: toDate(task.dueDate), 
                 estimated_hours: toNumeric(task.estimatedHours), assignees: task.assignees || [], 
                 subscribers: task.subscribers || [], auto_sla: task.autoSla ?? true, 
                 // Fix: Accessing customFields instead of custom_fields from task object
                 attachments: task.attachments || [], custom_fields: task.customFields || {}, 
                 external_id: task.externalId, meet_link: task.meetLink 
             };
             const { data, error } = await supabase.from('tasks').insert([payload]).select().single();
             if (error) throw error;
             return { 
                 ...data, clientId: data.client_id, startDate: data.start_date, dueDate: data.due_date, 
                 estimatedHours: data.estimated_hours, actualHours: data.actual_hours, 
                 autoSla: data.auto_sla, meetLink: data.meet_link, assignees: data.assignees || [], 
                 subscribers: data.subscribers || [], status: data.status as TaskStatus,
                 priority: data.priority as TaskPriority, customFields: data.custom_fields || {},
                 attachments: data.attachments || [], subtasks: [], comments: []
             };
        }
    ...
    createTasksBulk: async (tasks: Partial<Task>[]): Promise<void> => {
        if (isConfigured) {
            const payloads = tasks.map(t => ({
                title: t.title, description: t.description, client_id: toUUID(t.clientId),
                status: t.status, priority: t.priority, category: t.category,
                start_date: toDate(t.startDate), due_date: toDate(t.dueDate),
                // Fix: Accessing autoSla and meetLink instead of snake_case names from Partial<Task> object
                estimated_hours: toNumeric(t.estimatedHours), auto_sla: t.autoSla ?? true,
                external_id: t.externalId, meet_link: t.meetLink, assignees: t.assignees || [],
    ...
    updateTask: async (task: Task): Promise<Task> => {
        if (isConfigured) {
            const payload = {
                ...
                status: task.status, priority: task.priority, category: task.category,
                start_date: toDate(task.startDate), due_date: toDate(task.dueDate),
                estimated_hours: toNumeric(task.estimatedHours), actual_hours: toNumeric(task.actualHours),
                assignees: task.assignees || [], subscribers: task.subscribers || [],
                auto_sla: task.autoSla, custom_fields: task.customFields || {},
                is_tracking_time: task.isTrackingTime, last_time_log_start: task.lastTimeLogStart,
                external_id: task.externalId, meet_link: task.meetLink
            };
            ...
            return { 
                ...data, clientId: data.client_id, startDate: data.start_date, dueDate: data.due_date, 
                estimatedHours: data.estimated_hours, actualHours: data.actual_hours, 
                autoSla: data.auto_sla, meetLink: data.meet_link, status: data.status as TaskStatus,
                priority: data.priority as TaskPriority, assignees: data.assignees || [],
    ...
    createClient: async (client: Partial<Client>): Promise<Client> => {
        if (isConfigured) {
            const payload = {
                name: client.name, status: client.status, sla_tier_id: client.slaTierId,
                partner_id: client.partnerId, onboarding_date: client.onboardingDate,
                health_score: client.healthScore || 100, hours_used_month: client.hoursUsedMonth || 0,
                has_implementation: client.hasImplementation ?? true, billing_day: client.billingDay || 1,
    ...
    updateClient: async (client: Partial<Client>): Promise<Client> => {
        if (isConfigured && client.id) {
            const payload = {
                name: client.name, status: client.status, sla_tier_id: client.slaTierId,
                partner_id: client.partnerId, onboarding_date: client.onboardingDate,
                health_score: client.healthScore, hours_used_month: client.hoursUsedMonth,
                has_implementation: client.hasImplementation, billing_day: client.billingDay,
                custom_fields: client.customFields
            };
            const { data, error } = await supabase.from('clients').update(payload).eq('id', client.id).select().single();
            if (error) throw error;
            return { ...data, slaTierId: data.sla_tier_id, partnerId: data.partner_id, onboardingDate: data.onboarding_date, healthScore: data.health_score, hoursUsedMonth: d.hours_used_month, hasImplementation: d.has_implementation, billingDay: d.billing_day, customFields: d.custom_fields || {} };
        }
    ...
    createClientsBulk: async (clientsData: any[]): Promise<Client[]> => {
        if (isConfigured) {
            const payloads = clientsData.map(c => ({
                name: c.name, status: c.status || ClientStatus.ACTIVE, sla_tier_id: c.slaTierId,
                onboarding_date: c.onboarding_date || new Date().toISOString().split('T')[0],
                health_score: Number(c.healthScore) || 100, has_implementation: c.hasImplementation ?? true
            }));
    ...
    createPartner: async (partner: Partial<Partner>): Promise<Partner> => {
        if (isConfigured) {
            const payload = {
                name: partner.name, total_referrals: partner.totalReferrals || 0,
                ...
                cost_per_seat: partner.costPerSeat || 0, custom_fields: partner.customFields || {}
            };
    ...
    updatePartner: async (partner: Partial<Partner>): Promise<Partner> => {
        if (isConfigured && partner.id) {
            const payload = {
                name: partner.name, total_referrals: partner.totalReferrals,
                ...
                cost_per_seat: partner.costPerSeat, custom_fields: partner.customFields
            };
    ...
    createPartnersBulk: async (partnersData: any[]): Promise<Partner[]> => {
        if (isConfigured) {
            const payloads = partnersData.map(p => ({
                name: p.name, implementation_fee: Number(p.implementationFee) || 0,
    ...
    createTransaction: async (tr: Partial<Transaction>): Promise<Transaction> => {
        if (isConfigured) {
            const payload = { 
                date: tr.date, description: tr.description, category: tr.category, amount: tr.amount, 
                type: tr.type, status: tr.status, frequency: tr.frequency, installments: tr.installments, 
                client_id: tr.clientId, partner_id: tr.partnerId, custom_fields: tr.customFields || {}
            };
    ...
    createUser: async (user: Partial<User>): Promise<User> => {
        if (isConfigured) {
            const payload = {
                ...
                approved: user.approved ?? true, linked_entity_id: user.linkedEntityId,
    ...
    updateUser: async (user: User): Promise<User> => {
        if (isConfigured) {
            const payload: any = {
                name: user.name, email: user.email, role: user.role, approved: user.approved,
                linked_entity_id: user.linkedEntityId, permissions: user.permissions
            };
    ...
    createPlaybook: async (playbook: Partial<Playbook>): Promise<Playbook> => {
        if (isConfigured) {
            const payload = {
                title: playbook.title, client_id: playbook.clientId,
                blocks: playbook.blocks || [], theme: playbook.theme || {}
            };
    ...
    updatePlaybook: async (playbook: Playbook): Promise<void> => {
        if (isConfigured) {
            const payload = {
                title: playbook.title, client_id: playbook.clientId,
                blocks: playbook.blocks, theme: playbook.theme, updated_at: new Date().toISOString()
            };
